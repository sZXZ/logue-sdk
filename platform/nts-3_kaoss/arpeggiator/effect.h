...
public:
  ...
  fast_inline void Process(const float *in, float *out, size_t frames) {
    ...
    for (; out_p != out_e; in_p += 2, out_p += 2) {
      ...
      // Spec says: "unit providing a mix between the dry input and the
      // generated arpeggio signal." I'll mix it equally for now or just output
      // the synth. Most Kaoss sound sources are 100% wet when touched.
